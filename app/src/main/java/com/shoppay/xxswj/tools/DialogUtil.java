...
import com.shoppay.xxswj.adapter.DengjiChoseAdapter;
import com.shoppay.xxswj.adapter.TimeChoseAdapter;
import com.shoppay.xxswj.bean.Dengji;
import com.shoppay.xxswj.bean.JifenDk;
import com.shoppay.xxswj.bean.ShopCar;
import com.shoppay.xxswj.bean.VipPayMsg;
import com.shoppay.xxswj.db.DBAdapter;
import com.shoppay.xxswj.http.ContansUtils;
import com.shoppay.xxswj.http.InterfaceBack;
import com.shoppay.xxswj.view.wheelview.OnWheelChangedListener;
import com.shoppay.xxswj.view.wheelview.OnWheelScrollListener;
import com.shoppay.xxswj.view.wheelview.WheelView;
import com.shoppay.xxswj.view.wheelview.adapter.NumericWheelAdapter;
...
//				printReceipt_BlueTooth(context,xfmoney,yfmoney,jf,et_zfmoney,et_yuemoney,tv_dkmoney,et_jfmoney);
	...
	{
		String danhao = "换购单号:" +PreferenceHelper.readString(context, "shoppay","OrderAccount","");
		...
		String xfmoney = "换购积分:" +xfm;
		String obtainjifen = "获得酒瓶:" +objifen;
		...
		try
		{
			...
			tickname = "服务充次小票".getBytes("gb2312");
			 ...
			 tickname = "商品换购小票".getBytes("gb2312");
			}
			...
			byte[] yfmoney =( "应付积分:" +yfm).getBytes("gb2312");
			String js=(yfm-xfm)+"";
			byte[] jinshengmoney =( "节省积分:" +js).getBytes("gb2312");
			...
			if(isYue){
				byte[] yue=( "积分支付:" +et_yuemoney.getText().toString()).getBytes("gb2312");
				byte[][] mticketyue= {nextLine,left,yue};
				bytesList.add(ESCUtil.byteMerger(mticketyue));
			}
			if(isJifen){
				byte[] jifen=( "酒瓶抵扣:" +tv_dkmoney.getText().toString()).getBytes("gb2312");
			...
			byte[] syjinfen=( "剩余酒瓶:" +syjf).getBytes("gb2312");
...
//				double sy= Double.parseDouble(PreferenceHelper.readString(context, "shoppay", "MemMoney",""))-Double.parseDouble(et_yuemoney.getText().toString());
				byte[] shengyu=( "卡内积分:"+sy).getBytes("gb2312");
			...
			byte[] time=( "换购时间:"+getStringDate()).trim().getBytes("gb2312");
...
//			Log.d("xx","异常");
	...
	public static byte[] printReceipt_BlueTooth(final String type,Context context,VipPayMsg msg)
	{
		...
		String huiyuankahao = "会员卡号:" +PreferenceHelper.readString(context, "shoppay", "vipcar","无");
		String huiyuanming = "会员名称:" +PreferenceHelper.readString(context, "shoppay", "vipname", "散客");
		String xfmoney = "换购积分:" +StringUtil.twoNum(msg.xfMoney);
		String obtainjifen = "获得酒瓶:" +(int)Double.parseDouble(msg.obtainJifen);
		Log.d("xx",PreferenceHelper.readString(context, "shoppay", "vipname", "散客"));
		try
		{
			byte[] next2Line = ESCUtil.nextLine(2);
			...
			byte[] yfmoney =( "应付积分:" +StringUtil.twoNum(msg.zhMoney)).getBytes("gb2312");
			byte[] jinshengmoney =( "节省积分:" +StringUtil.twoNum(msg.jieshengMoney)).getBytes("gb2312");
			...
			if(isYue){
				byte[] yue=( "积分支付:" +StringUtil.twoNum(msg.yueMoney)).getBytes("gb2312");
				byte[][] mticketyue= {nextLine,left,yue};
				bytesList.add(ESCUtil.byteMerger(mticketyue));
			}
			if(isWx){
			...
			if(isJifen){
				byte[] jifen=( "酒瓶抵扣:" +msg.jifenDkmoney).getBytes("gb2312");
				byte[][] mticketjin= {nextLine,left,jifen};
				bytesList.add(ESCUtil.byteMerger(mticketjin));
			}
			if(!msg.vipName.equals("散客")){
				byte[] syjinfen=( "剩余酒瓶:" +(int)Double.parseDouble(msg.vipSyJifen)).getBytes("gb2312");
				byte[][] mticketsyjf= {nextLine,left,syjinfen};
				bytesList.add(ESCUtil.byteMerger(mticketsyjf));
			}
			if(isYue){
//				double sy=CommonUtils.del(Double.parseDouble(PreferenceHelper.readString(context, "shoppay", "MemMoney","")),Double.parseDouble(et_yuemoney.getText().toString()));
				byte[] shengyu=( "卡内积分:"+StringUtil.twoNum(msg.vipYue)).getBytes("gb2312");
				byte[][] mticketsy= {nextLine,left,shengyu};
				bytesList.add(ESCUtil.byteMerger(mticketsy));
			}
