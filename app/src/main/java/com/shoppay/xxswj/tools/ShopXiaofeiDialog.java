...
public class ShopXiaofeiDialog {
    ...
    public static String vippwd="";

    public static Dialog jiesuanDialog(MyApplication app, final String vipphone, final  boolean isVip, final Dialog loading, final Context context,
                            ...
                            if(sysquanxian.ispassword == 1) {
                                DialogUtil.pwdDialog(context, 1, new InterfaceBack() {
                                    ...
                                    public void onResponse(Object response) {
                                        vippwd=response.toString();
                                        if(sysquanxian.isphone==1){
                                            if(vipphone.equals("")){
                                                Toast.makeText(MyApplication.context, "该会员没有登记手机号，不能积分换购",
                                                        Toast.LENGTH_SHORT).show();
                                            }else {
                                                CodeDialog.pwdDialog(context, dialog, vipphone, 1, new InterfaceBack() {
                                                    @Override
                                                    public void onResponse(Object response) {
                                                        jiesuan(loading, type, handler, dialog,context, vippwd,DateUtils.getCurrentTime("yyyyMMddHHmmss"));
                                                    }
